...
public class AndroidQTransformUtils {
     ...
     * #耗时操作需要放在子线程中操作
     */
    private static String parseVideoPathToAndroidQ(Context ctx, String path, String customFileName, String mineType) {
        ...
        try {
            ...
            FileInputStream inputStream = new FileInputStream(Objects.requireNonNull(parcelFileDescriptor).getFileDescriptor());
            String md5Value = Digest.computeToQMD5(inputStream);
            String fileName;
            if (!TextUtils.isEmpty(md5Value)) {
                fileName = TextUtils.isEmpty(customFileName) ? "VID_" + md5Value.toUpperCase() + suffix : customFileName;
            } else {
    ...
    public static String parseImagePathToAndroidQ(Context ctx, String path, String customFileName, String mineType) {
        ...
        try {
            ...
            String fileName;
            if (!TextUtils.isEmpty(md5Value)) {
                fileName = TextUtils.isEmpty(customFileName) ? "IMG_" + md5Value.toUpperCase() + suffix : customFileName;
            } else {
     ...
     * #耗时操作需要放在子线程中操作
     */
    private static String parseAudioPathToAndroidQ(Context ctx, String path, String customFileName, String mineType) {
        ParcelFileDescriptor parcelFileDescriptor = null;
        try {
            ...
            String fileName;
            if (!TextUtils.isEmpty(md5Value)) {
                fileName = TextUtils.isEmpty(customFileName) ? "AUD_" + md5Value.toUpperCase() + suffix : customFileName;
            } else {
    ...
    public static void copyPathToDCIM(Context context, Uri inputUri, Uri outUri) {
        ...
        try {
            ...
            while ((read = inputStream.read(buffer)) > -1) {
                Objects.requireNonNull(outputStream).write(buffer, 0, read);
            }
